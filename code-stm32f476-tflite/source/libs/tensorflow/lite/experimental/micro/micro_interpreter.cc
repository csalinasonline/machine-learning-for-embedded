...
MicroInterpreter::MicroInterpreter(const Model* model,
  ...
  for (flatbuffers::uoffset_t i = 0; i < subgraph_->inputs()->Length(); ++i) {
  ...
  int* last_used = reinterpret_cast<int*>(tensor_allocator_->AllocateMemory(
      sizeof(int) * tensors_->Length(), sizeof(int)));
  for (flatbuffers::uoffset_t i = 0; i < tensors_->Length(); ++i) {
    first_created[i] = -1;
    last_used[i] = -1;
  }
  ...
  for (int i = (operators_->Length() - 1); i >= 0; --i) {
    ...
    for (flatbuffers::uoffset_t n = 0; n < op->inputs()->Length(); ++n) {
      const int tensor_index = op->inputs()->Get(n);
      if ((last_used[tensor_index] == -1) || (last_used[tensor_index] < i)) {
        last_used[tensor_index] = i;
      }
    }
    for (flatbuffers::uoffset_t n = 0; n < op->outputs()->Length(); ++n) {
...
TfLiteStatus MicroInterpreter::Invoke() {
  ...
  for (flatbuffers::uoffset_t i = 0; i < operators_->Length(); ++i) {
    const auto* op = operators_->Get(i);
    flatbuffers::uoffset_t index = (flatbuffers::uoffset_t) op->opcode_index();
...
TfLiteTensor* MicroInterpreter::output(int index) {
  ...
  if ((index < 0) || (index >= (int) outputs->Length())) {
